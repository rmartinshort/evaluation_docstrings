...
class LLMStructuredOutputCaller(ABC):
    ...
    def invoke(self, input_string, system_template, response_template, input_model_name, temperature=0):
    ...
    def craft_input(string_input, system_template):
        ...
        messages = [
            {
                "role": "system", "content": system_template.system_template,
            },
            {
...
class OpenAIStructuredOutputCaller(LLMStructuredOutputCaller):
    ...
    def set_up_client(self):
        client = instructor.from_openai(OpenAI(api_key=self.api_key), mode=instructor.Mode.TOOLS_STRICT)
        return client
